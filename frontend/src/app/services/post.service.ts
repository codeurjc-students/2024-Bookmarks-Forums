...
export class PostService {
    ...
    searchPosts(query: string, page: number, size: number, order: string): Observable<Map<string, Object>> {
    ...
    getCommunityPosts(communityID: number, count: boolean, page: number, size: number, sort: string, query?: string): Observable<Map<string, Object>> {
        let params = new HttpParams()
            .set('count', count)
            .set('page', page)
            .set('size', size)
        ...
        return this.http
            .get<Map<string, Object>>(`${COMMUNITY_API_URL}/${communityID}/posts`, { params: params })
            .pipe(catchError((error) => throwError(() => error)));
    }
    ...
    editPost(postId: number, postDTO: FormData, action: string): Observable<Post> {
    ...
    updatePostImage(postId: number, image: File, action?: string): Observable<Post> {
    ...
    getRepliesOfPost(postId: number, page: number, size: number, order: string): Observable<Map<string, Object>> {
        let params = new HttpParams()
            .set('page', page)
            .set('size', size)
            .set('order', order);
        return this.http
            .get<Map<string, Object>>(`${API_URL}/${postId}/replies`, { params: params })
            .pipe(catchError((error) => throwError(() => error)));
    }
    ...
    searchReplies(criteria: string, query: string, page: number, size: number): Observable<Map<string, Object>> {
        let params = new HttpParams()
    ...
    createReply(postId: number, replyData: { [key: string]: string }): Observable<any> {
        return this.http
    ...
    */
    getUserRecommendations(option: string){
        switch(option){
            case 'most-liked-users':
                return this.http
    ...
    */
    getGeneralRecommendations(option: string){
        switch(option){
            case 'most-liked-users':
                return this.http
                .get<Post[]>(`${USER_API_URL}/posts/most-liked`)
                .pipe(catchError((error) => throwError(() => error)));
            case 'most-liked-communities':
                return this.http
